...
def add_cart(request):
    if request.method == 'POST':
        #加入到购物车，需判断用户是否登录
        #如果登录，加入到购物车中的数据，其实就是加入到数据库中购物车表中

        #如果登录，加入到购物车中的数据存储到session中
        #如果没有登录，则加入到购物车的数据是加入到session中
        # session中存储数据，商品id，商品数量，商品的选择状态
        #如果登录，则把session中的数据同步到数据库中(中间价同步数据)
        #1.获取商品id和商品数量
...
def cart(request):
    if request.method == 'GET':
        #如果没有登录，则从session中获取商品的信息
        #如果登录，还是从session中获取数据（保证数据库中的商品和session中商品一致）
        ...
        if session_goods:
            ...
            #后台返回结构[[goods objects,number,total_price],[goods_objects]]
            #获取商品对象
...
def place_order(request):
    if request.method == 'GET':
        ...
        for cart in carts:
            ...
            cart.total_price = int(cart.nums) \
                               * int(cart.goods.shop_price)
...
def f_price(request):
    ...
    {key:[[id1,price1],[id2,price2]],key2:total price}
    """
    if request.method == 'GET':
        user_id = request.session.get('user_id')
        if user_id:
            ...
            #获取session中所有的商品信息，[id,num,is_select]
            session_goods = request.session.get('goods')
            ...
            cart_data['all_price'] = all_price
        return JsonResponse({'code':200,'cart_data':cart_data,'all_price':all_price})
...
def change_goods_num(request):
    if request.method == 'POST':
        #修改购物车中商品的个数
        #1.先判断用户是否登录，如果没有登录，则修改session中的值
        #2.如果用户已经登录，需要判断当前修改的商品是否存在于session中，
        # 如果存在则修改session，不存在就修改购物车的表
        ...
        if session_goods:
            for goods in session_goods:
                if goods_id == goods[0]:
                    ...
                    goods[2] = is_select
            request.session['goods'] = session_goods
        #如果用户登录了，则需要在购物车中修改数据，因为session中的商品有可能不在购物车表中
        if user_id:
            #修改购物车中商品的个数
...
def del_goods(request):
    if request.method == 'POST':
        #删除购物车中的商品
        #如果用户没有登录，删除session中的值，如果用户登录则删除数据库中的数据
        #获取商品
        goods_id = request.POST.get('goods_id')
        user_id = request.session.get('user_id')
        ...
        if user_id:
            request.session.flush(session_goods)
